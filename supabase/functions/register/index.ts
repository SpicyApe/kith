// index.ts — HTTP entrypoint for `register`. Wires the pure handler to Deno.serve.

import { handleRegister } from "./handler.ts";
import { SupabaseRegisterStore } from "./store.ts";
import {
  badJson,
  buildContext,
  internalError,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const store = new SupabaseRegisterStore(supabaseUrl, serviceRoleKey);
    const outcome = await handleRegister(parsed.value, ctx, store, pepper);
